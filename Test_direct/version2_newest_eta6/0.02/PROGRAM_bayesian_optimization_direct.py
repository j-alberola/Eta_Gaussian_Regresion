...
#    
#    # Normalize input
#    x_norm = (x - X_mean_old) / X_std_old
#    
#    # Compute covariance vector k_star
#    k_star = kernel.K(x_norm, X)
#    k_star_star = kernel.K(x_norm, x_norm)
#    
#    # Mean derivative
#    dk_dx = - (x - X) / (lengthscale ** 2) * k_star  # Kernel derivative
#    d_mean_dx = dk_dx @ alpha
#    
#    # Covariance derivative
#    L = np.linalg.cholesky(K + np.eye(len(X)) * 1e-11)
#    v = cho_solve((L, True), k_star.T)
#    d_sigma_dx = - (dk_dx @ v) / np.sqrt(k_star_star - k_star @ v)
#    
#    return d_mean_dx - kappa * d_sigma_dx
#

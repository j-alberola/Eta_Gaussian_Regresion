...
for i in {1..10}; do
    ...
    mkdir version2_newest_eta$i
    cp -r N2_minus.hf.4states.no.cipsi.17200 version2_newest_eta$i
    cp Good_columns.py version2_newest_eta$i
    cp PROGRAM* version2_newest_eta$i
    cp datafile_big_scan.dat version2_newest_eta$i
    cp change_kappa.sh version2_newest_eta$i
    cp script* version2_newest_eta$i

    cd version2_newest_eta$i
    ./change_kappa.sh $i

    ./script_scan_version_2.sh N2_minus.hf.4states.no.cipsi.17200
...
#    awk -v i="$i" 'END {print "0.000_0.0085", i, "LB", $0}' N2_minus.hf.4states.no.cipsi.17200.scan.out.data1.LB.good >> ../CONVERGED_MINIMA
    awk -v i="$i" 'END {print "0.000_0.0085", i, "LB", $0}' Last_point_LB  >> ../CONVERGED_MINIMA_NEW
...
#        awk -v j="$j" -v i="$i" 'END {print "0.000_" j, i, "EI", $0}' N2_minus.hf.4states.no.cipsi.17200.scan.out.data1.EI.good >> ../../CONVERGED_MINIMA
        awk -v j="$j" -v i="$i" 'END {print "0.000_" j, i, "LB", $0}' Last_point_LB >> ../../CONVERGED_MINIMA_NEW

...
for i in {1..10}; do
    ...
    mkdir version_original_newest_eta$i
    cp -r N2_minus.hf.4states.no.cipsi.17200 version_original_newest_eta$i
    cp Good_columns.py version_original_newest_eta$i
    cp PROGRAM* version_original_newest_eta$i
    cp datafile_big_scan.dat version_original_newest_eta$i
    cp change_kappa_original.sh version_original_newest_eta$i
    cp script* version_original_newest_eta$i

    cd version_original_newest_eta$i
    ./change_kappa_original.sh $i

    ./script_scan_version_original.sh N2_minus.hf.4states.no.cipsi.17200
...
#    awk -v i="$i" 'END {print "0.000_0.0085", i, "LB", $0}' N2_minus.hf.4states.no.cipsi.17200.scan.out.data1.LB.good >> ../CONVERGED_MINIMA
    awk -v i="$i" 'END {print "0.001_0.0085", i, "LB", $0}' Last_point_LB  >> ../CONVERGED_MINIMA_ORIGINAL
...
#        sed -i "s/qp set cap eta_cap 0.001/qp set cap eta_cap 0.000/" script_scan_version_2.sh
        sed -i "s/qp set cap eta_step_size 0.0075/qp set cap eta_step_size $j/" script_scan_version_original.sh
        ./script_scan_version_original.sh N2_minus.hf.4states.no.cipsi.17200

#        awk -v j="$j" -v i="$i" 'END {print "0.000_" j, i, "EI", $0}' N2_minus.hf.4states.no.cipsi.17200.scan.out.data1.EI.good >> ../../CONVERGED_MINIMA
        awk -v j="$j" -v i="$i" 'END {print "0.001_" j, i, "LB", $0}' Last_point_LB >> ../../CONVERGED_MINIMA_ORIGINAL

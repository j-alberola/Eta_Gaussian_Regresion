...
for x_star in new_points:
    ...
    # Calculate the derivative for this specific point
    Acquisition_LB_deriv = d_LB_acquisition(4.0,x_star_reshaped)
...
def wrapper(x):
    return d_LB_acquisition(kappa,x)
#new_points = np.linspace(x_min, x_max, 10000).reshape(-1, 1)
...
def minimize_LB(verbose=False):
    """Find the minimum of the LB acquisition function with optional verbose output."""
    lbfgs_options = {
        "gtol": 1e-8,   # Stricter gradient tolerance
        "ftol": 1e-12,  # Stricter function tolerance
        "maxiter": 5000  # More iterations for stability
    }
    
    # Ensure best_root is valid
    if best_root is None:
        raise ValueError("Error: best_root is None. Provide a valid initial guess.")

    if verbose:
        print(f"Starting L-BFGS-B minimization with initial guess: {best_root}")

    # Define a callback function to track progress
    def callback(xk):
        if verbose:
            print(f"Iteration update: Current x = {xk}")
    ...
    result = minimize(
        ...
        bounds=bounds,
        method="L-BFGS-B",
        options={"eps": 1e-8, "gtol": 1e-10, "ftol": 1e-15, "maxiter": 10000},
        callback=callback if verbose else None  # Add callback only if verbose is True
    )

    if verbose:
        print(f"Optimization completed.\nMinimum found at x = {result.x}, LB value = {result.fun}")
        print(f"Convergence status: {result.message}")

    return result.x, result.fun  # Return the local minimum

# Example usage
x_min_LB, min_LB_value = minimize_LB(verbose=True)

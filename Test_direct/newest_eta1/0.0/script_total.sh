...
for i in {2..9}; do
    ...
    awk -v i="$i" 'END {print "0.001_0.0085", i, "LB", $0}' N2_minus.hf.4states.no.cipsi.17200.scan.out.data1.LB.good >> ../CONVERGED_MINIMA
    

    for j in 0.012 0.015 0.02 0.025 0.03; do
        mkdir $j
...
#        sed -i "s/qp set cap eta_cap 0.001/qp set cap eta_cap 0.000/" script_scan_version_2.sh
        ...
        awk -v j="$j" -v i="$i" 'END {print "0.000_" j, i, "LB", $0}' N2_minus.hf.4states.no.cipsi.17200.scan.out.data1.LB.good >> ../../CONVERGED_MINIMA

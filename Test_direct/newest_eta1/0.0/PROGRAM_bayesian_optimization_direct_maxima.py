...
def find_first_maximum_acq(x_range, kappa):
    """Find the first local maximum of the LB acquisition function."""
    values = [LB_acquisition(kappa, np.atleast_2d(x)) for x in x_range]
    for i in range(1, len(values) - 1):
        if values[i] > values[i - 1] and values[i] > values[i + 1]:  # Local max
            return x_range[i]
    return x_range[0]  # If no clear max, return the first point

def multi_start_minimize_LB(n_starts, kappa, bounds, x_min, x_max):
    """Find the global minimum of the LB acquisition function after the first maximum."""
    best_x = None
    best_fun = float("inf")

    # Define a fine-grained grid to search for the first maximum
    x_range = np.linspace(x_min, x_max, 100)
    first_max_x = find_first_maximum_acq(x_range, kappa)

    # Adjust bounds to exclude points before the first maximum
    new_bounds = [(first_max_x, x_max)]

    for _ in range(n_starts):
        x0 = np.random.uniform(first_max_x, x_max, size=(1,))  # Start only after first max
        result = minimize(
            lambda x: LB_acquisition(kappa, np.atleast_2d(x)),
            x0,
            bounds=new_bounds,
...
x_min_LB, min_LB_value = multi_start_minimize_LB(100,2.0,bounds, x_min, x_max)

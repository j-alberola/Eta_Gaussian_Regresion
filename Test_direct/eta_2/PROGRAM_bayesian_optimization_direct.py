...
data_big = np.loadtxt('datafile_very_big_scan.dat')
...
L = cholesky(K + np.eye(len(X)) * 1e-12, lower=True)  # Add jitter for stability
...
def gp_surrogate_covariance(x_star):
    ...
    L = cholesky(K + np.eye(len(X)) * 1e-12, lower=True)
...
x_max = np.max(X_old)
new_points = np.linspace(x_min, x_max, 1000).reshape(-1, 1)
new_points2 = np.linspace(x_min, x_max, 1000).reshape(-1, 1)
...
Y_list_cov = np.array([gp_surrogate_covariance(x.reshape(1, 1)) for x in new_points2]).flatten()

std_dev = np.sqrt(Y_list_cov)
upper_bound = Y_list_lines + 1.96 * std_dev
lower_bound = Y_list_lines - 1.96 * std_dev
...
plt.fill_between(new_points2.flatten(), lower_bound, upper_bound, color='b', alpha=0.2, label="95% Confidence Interval")
plt.scatter(X_old, Y_list_training.flatten(), color='g', marker='x', label="Regression at training point")  # Original data points
plt.scatter(X_old, velocity_old, color='r', marker='x', label="Training Data")  # Original data points
plt.scatter(X_big, velocity_big, color='k', marker='x', label="Big Scan")
